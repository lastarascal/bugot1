...
if(file_exists('home/fileindex.html')){
	$tampil = file_get_contents('home/fileindex.html');
	echo $tampil;
}else{
...
if($result->num_rows > 0){
	...
	if( $i < 11 ){
		...
		$contents .= '<article id="post-'.$postnum.'" class="box-post ktz-archive post-'.$postnum.' post type-post status-publish format-standard has-post-thumbnail hentry"><h2 class="entry-title"><a href="'.$siteurl.'/'.$id.'/'.$titleurl.'" title="'.$title.'" rel="bookmark">'.$title.'</a></h2><div class="meta-post"></div><div class="entry-body media"><div class="clearfix"><div class="ktz-featuredimg"><a href="'.$siteurl.'/'.$id.'/'.$titleurl.'"><img width="560" height="386" src="'.preg_replace('/_SS[0-9]{3}_\.jpg/','_SS250_.jpg',$item->books[0]->image).'" class="attachment-full wp-post-image" alt="'.$title.'" /></a></div></div><div class="media-body ktz-post">'.batasChar($item->books[0]->description, 250).'</div></div></article>';
		$i++;
		$postnum++;
	}elseif( $i < 21){
